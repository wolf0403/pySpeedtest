...
class Counter(object):
    ...
    def add(self, size, lastrec=False):
        ...
        if delta >= self.delta or lastrec:
            ...
            rec = (
                self.size + size, secs, self.size / secs, t, delta >= self.delta)
            if DEBUG:
                print "ADD: ", rec
...
class AsyncHTTP(asyncore.dispatcher):
    ...
    def __init__(self, uris, consumer, counter=None, pipelining=False):
        ...
        self.requests[-1] = self.requests[-1][:-2] + \
            "Connection: close\r\n\r\n"
    ...
    def _do_one_req(self):
        if len(self.requests) > 0:
            ...
            if sent < len(req0):
                ...
                # print "do_req {:d}, {:d} left".format(self.do_req,
                # len(self.requests))
...
if __name__ == '__main__':
    ...
    if True:  # shared counter
        counter = Counter()
        m, t, s, c = multi_get(
            [[e] for e in sys.argv[1:]], sharedCounter=counter)
        recs = counter.records
    else:
        counter = None
        m, t, s, counters = multi_get(
            [[e] for e in sys.argv[1:]])  # multi socket
        recs = None
